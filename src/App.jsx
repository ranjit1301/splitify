...
const auth = {
  ...
  // Step 2: verify the 6-digit OTP from email
  async verifyEmailOTP(email, token) {
    const res = await supaAuth("POST", "/verify", { type: "email", email, token });
    if (res?.access_token) {
    ...
    return { success: false, error: res?.error_description || res?.msg || "Invalid OTP" };
  },
  restore() {
...
const db = {
  ...
  async getMyTrips() {
    ...
    const created = await supaFetch("GET", `/trips?created_by=eq.${_userId}&order=created_at.desc`) || [];
    // Get trips user is invited to via email
    if (!_userEmail) return created;
    const memberships = await supaFetch("GET", `/trip_members?email=eq.${encodeURIComponent(_userEmail)}`) || [];
    const invitedIds = memberships.map(m => m.trip_id).filter(id => !created.find(t => t.id === id));
    let invited = [];
    if (invitedIds.length > 0) {
      invited = await supaFetch("GET", `/trips?id=in.(${invitedIds.join(",")})`) || [];
    }
    return [...created, ...invited].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  },
          ...
          {step === "email" ? (
            ...
            <>
              ...
              <div style={{ background:"rgba(16,185,129,0.07)", border:"1px solid rgba(16,185,129,0.2)", borderRadius:10, padding:"0.7rem 0.9rem", marginBottom:"1.2rem", fontSize:13, color:"#6BCB77" }}>
                📬 Open your email app and find the email from Supabase — enter the 6-digit code below
              </div>
              <div style={{ marginBottom:"1.4rem" }}>
                <label style={{ fontSize:11, color:"#7a6f9a", fontWeight:700, display:"block", marginBottom:8, textTransform:"uppercase", letterSpacing:1 }}>6-Digit Code</label>
                <input value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,""))} placeholder="123456" maxLength={6} type="tel"
                  onKeyDown={e=>e.key==="Enter"&&verifyOTP()}
                  style={{ ...iStyle, fontSize:32, letterSpacing:10, textAlign:"center", fontWeight:800 }}/>
              </div>
              {error && <div style={{ color:"#FF6B6B", fontSize:13, marginBottom:"1rem", fontWeight:600, background:"rgba(255,107,107,0.08)", borderRadius:8, padding:"0.5rem 0.8rem" }}>⚠️ {error}</div>}
...
function TripDetail({ trip, onBack, showToast, currentUserName }) {
  ...
  const inviteMember=async()=>{
    if(!inviteForm.name.trim()||!inviteForm.phone.trim()){showToast("Enter name and phone","error");return;}
    setSavingInv(true);
    ...
    if(result){
      ...
      const color=COLORS[members.length%COLORS.length];
      const mem=await db.addMember({trip_id:trip.id,name:inviteForm.name.trim(),whatsapp:inviteForm.phone.trim(),color});
      if(mem) setMembers(p=>[...p,...(Array.isArray(mem)?mem:[mem])]);
  ...
  const recordPayment=async(txn,amount)=>{
    ...
    if(result){setPayments(p=>[...p,...(Array.isArray(result)?result:[result])]);showToast(`₹${amount.toFixed(2)} recorded! ✅`);}
...
/* ══════════════════════════════════════
   HOME SCREEN
══════════════════════════════════════ */
function HomeScreen({ trips,loading,onSelectTrip,onCreateTrip,onDeleteTrip,showToast,userName,onLogout }) {
  ...
  const createTrip=async()=>{
    ...
    if(result){onCreateTrip(Array.isArray(result)?result[0]:result);showToast(`${form.name.trim()} created! 🎉`);}
    ...
    <div style={{ minHeight:"100vh",background:"#0e0b1e",fontFamily:"'Outfit',sans-serif",color:"#e8e8f0",display:"flex",flexDirection:"column",alignItems:"center" }}>
      ...
      {!authed
        ...
        : activeTrip&&currentTrip
          ...
          : <HomeScreen trips={trips} loading={loading} onSelectTrip={setActiveTrip} onCreateTrip={t=>setTrips(p=>[t,...p])} onDeleteTrip={deleteTrip} showToast={showToast} userName={userName} onLogout={handleLogout}/>
      }
    </div>
  );
}
