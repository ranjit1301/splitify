...
function TripDetail({ trip, onBack, showToast, currentUserName }) {
  ...
  const [memberForm,setMemberForm]=useState({name:"",whatsapp:"",upiId:""});
  ...
  const addMember=async()=>{
    ...
    const result=await db.addMember({trip_id:trip.id,name:memberForm.name.trim(),whatsapp:memberForm.whatsapp.trim(),upi_id:memberForm.upiId.trim(),color});
    if(result){showToast(`${memberForm.name.trim()} added! 👋`);}
    else showToast("Failed to add member","error");
    setMemberForm({name:"",whatsapp:"",upiId:""}); setShowAddMember(false); setSavingMem(false);
  };
  ...
  // UPI payment link — uses saved UPI ID if available, else phone@ybl
  const getUpiId=(member)=>{
    if(member.upi_id&&member.upi_id.trim()) return member.upi_id.trim();
    const phone=member.whatsapp?.replace(/\D/g,"").slice(-10)||"";
    return phone?`${phone}@ybl`:"";
  };

  const upiLink=(creditor,amount,tripName)=>{
    const pa=getUpiId(creditor);
    if(!pa) return "#";
    const note=`Splitify ${tripName}`;
    return `upi://pay?pa=${encodeURIComponent(pa)}&pn=${encodeURIComponent(creditor.name)}&am=${amount.toFixed(2)}&cu=INR&tn=${encodeURIComponent(note)}`;
  };

  // Check if logged-in user matches a member (by email/name)
  // Let user pick which member they are (stored per trip)
  const myMemberKey = `splitify_me_${trip.id}`;
  const [myMemberId, setMyMemberId] = useState(()=>localStorage.getItem(myMemberKey)||null);
  const selectMe = (memberId) => {
    setMyMemberId(memberId);
    localStorage.setItem(myMemberKey, memberId);
  };
  const isCurrentUserMember=(member)=> member.id === myMemberId;
    ...
    <div style={{ width:"100%",maxWidth:480,display:"flex",flexDirection:"column",height:"100vh" }}>
          ...
          <div style={{ padding:"1rem 1.2rem" }}>
            ...
            {members.length===0?(
              ...
              <div key={m.id} style={{ background:"linear-gradient(135deg,#1e1540,#18122e)",borderRadius:14,padding:"0.9rem 1rem",marginBottom:"0.7rem",display:"flex",alignItems:"center",gap:12,border:"1px solid rgba(124,58,237,0.15)" }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:700,fontSize:15,color:"#e8e0ff" }}>{m.name}</div>
                  {m.whatsapp&&<div style={{ fontSize:12,color:"#6BCB77",marginTop:2 }}>📱 {m.whatsapp}</div>}
                  {m.upi_id&&<div style={{ fontSize:12,color:"#a78bfa",marginTop:2 }}>💳 {m.upi_id}</div>}
                </div>
          ...
          <div style={{ padding:"1rem 1.2rem" }}>
            ...
            {members.length===0?(
              ...
              <>
                ...
                {/* Who am I selector for UPI */}
                <div style={{ background:"rgba(124,58,237,0.08)",border:"1px solid rgba(124,58,237,0.2)",borderRadius:12,padding:"0.7rem 1rem",marginBottom:"0.8rem" }}>
                  <div style={{ fontSize:11,color:"#a78bfa",fontWeight:700,marginBottom:8,textTransform:"uppercase",letterSpacing:0.8 }}>👤 I am (select yourself for Pay Now)</div>
                  <div style={{ display:"flex",flexWrap:"wrap",gap:6 }}>
                    {members.map(m=>(
                      <button key={m.id} onClick={()=>selectMe(m.id)} style={{ display:"flex",alignItems:"center",gap:5,background:myMemberId===m.id?m.color+"2a":"rgba(255,255,255,0.04)",border:`1.5px solid ${myMemberId===m.id?m.color:"rgba(124,58,237,0.15)"}`,color:myMemberId===m.id?m.color:"#7a6f9a",borderRadius:20,padding:"4px 12px 4px 6px",fontSize:12,cursor:"pointer",fontFamily:"'Outfit',sans-serif",fontWeight:600 }}>
                        <Avatar name={m.name} color={m.color} size={20}/>{m.name}{myMemberId===m.id?" ✓":""}
                      </button>
                    ))}
                  </div>
                  {!myMemberId&&<div style={{ fontSize:11,color:"#FF6B6B",marginTop:6,fontWeight:600 }}>⬆️ Select yourself to see your Pay Now buttons</div>}
                </div>
                {settlements.txns.length===0?(
                  ...
                  return (
                    <div key={i} style={{ background:"linear-gradient(135deg,#1e1540,#1a0d36)",borderRadius:16,padding:"1.2rem",marginBottom:"0.9rem",border:"1px solid rgba(124,58,237,0.25)" }}>
                      ...
                      {/* iAmDebtor = logged-in user is the one who owes */}
                      {(()=>{ const iAmDebtor=isCurrentUserMember(dm); return (
                      <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                        ...
                        {iAmDebtor&&getUpiId(cm)?(
                          <div style={{ flex:1,display:"flex",flexDirection:"column",gap:4 }}>
                            <div style={{ fontSize:10,color:"#7a6f9a",textAlign:"center" }}>
                              Paying to: <span style={{ color:"#a78bfa",fontWeight:700 }}>{getUpiId(cm)}</span>
                            </div>
                            <a href={upiLink(cm,t.amount,trip.name)} style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:6,background:"linear-gradient(135deg,#7C3AED,#10B981)",color:"#fff",borderRadius:10,padding:"0.6rem",textDecoration:"none",fontWeight:800,fontSize:13,fontFamily:"'Outfit',sans-serif",boxShadow:"0 4px 16px rgba(124,58,237,0.4)" }}>💳 Pay ₹{t.amount.toFixed(2)} via UPI</a>
                          </div>
                        ):iAmDebtor&&!getUpiId(cm)?(
                          <div style={{ flex:1,background:"rgba(255,193,7,0.08)",border:"1px solid rgba(255,193,7,0.2)",borderRadius:10,padding:"0.5rem",textAlign:"center",fontSize:11,color:"#FFC107",fontWeight:600 }}>
                            ⚠️ {cm.name} hasn't added UPI ID yet
                          </div>
                        ):null}
                        {!iAmDebtor&&dm.whatsapp?(
      ...
      {showAddMember&&(
        <Modal onClose={()=>setShowAddMember(false)}>
          ...
          {[{label:"Name *",key:"name",placeholder:"e.g. Rahul",type:"text"},{label:"WhatsApp Number",key:"whatsapp",placeholder:"+91 9876543210",type:"tel"},{label:"UPI ID (for payments)",key:"upiId",placeholder:"e.g. 9876543210@ybl or name@okicici",type:"text"}].map(f=>(
          ...
          <div style={{ background:"rgba(124,58,237,0.07)",border:"1px solid rgba(124,58,237,0.15)",borderRadius:10,padding:"0.6rem 0.8rem",marginBottom:"0.5rem",fontSize:12,color:"#7a6f9a" }}>
            💡 UPI ID can be found in any UPI app → Profile → UPI IDs
          </div>
