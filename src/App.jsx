...
function LoginScreen({ onLogin }) {
  ...
  const verifyOTP = async () => {
    ...
    if (otp.length < 8) { setError("Enter the OTP code from your email"); return; }
    setLoading(true); setError("");
          ...
          {step === "email" ? (
            ...
            <>
              ...
              <div style={{ marginBottom:"1.4rem" }}>
                ...
                <input value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,""))} placeholder="12345678" maxLength={8} type="tel"
                  onKeyDown={e=>e.key==="Enter"&&verifyOTP()}
                  style={{ ...iStyle, fontSize:24, letterSpacing:5, textAlign:"center", fontWeight:800 }}/>
              </div>
              {error && <div style={{ color:"#FF6B6B", fontSize:13, marginBottom:"1rem", fontWeight:600, background:"rgba(255,107,107,0.08)", borderRadius:8, padding:"0.5rem 0.8rem" }}>⚠️ {error}</div>}
