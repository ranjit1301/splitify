...
const auth = {
  // Step 1: send OTP code to email (no redirect = forces numeric code)
  async sendEmailOTP(email) {
    // Passing no redirectTo forces Supabase to send a numeric OTP
    // instead of a magic link
    return supaAuth("POST", "/otp", {
      email,
      create_user: true,
    });
  },
  // Step 2: verify the 8-digit OTP code from email
  async verifyEmailOTP(email, token) {
    // Try "email" type first, then "signup" type as fallback
