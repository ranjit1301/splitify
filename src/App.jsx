...
function TripDetail({ trip, onBack, showToast, currentUserName }) {
  ...
  // UPI deep link - uses phone number with ybl (most common)
  const upiLink=(creditor,amount,tripName)=>{
    const phone=creditor.whatsapp.replace(/\D/g,"").slice(-10);
    const note=`Splitify-${tripName}`;
    // BHIM UPI format works across all apps
    return `upi://pay?pa=${phone}%40ybl&pn=${encodeURIComponent(creditor.name)}&am=${amount.toFixed(2)}&cu=INR&tn=${encodeURIComponent(note)}&mode=02&purpose=00`;
  };

  // Check if logged-in user matches a member (by email/name)
  const isCurrentUserMember=(member)=>{
    // Match by email stored in whatsapp field (from invite) or by name
    const userEmail = auth.email()||"";
    const userPhone = member.whatsapp||"";
    return userPhone.toLowerCase()===userEmail.toLowerCase() || 
                  ...
                  return (
                    <div key={i} style={{ background:"linear-gradient(135deg,#1e1540,#1a0d36)",borderRadius:16,padding:"1.2rem",marginBottom:"0.9rem",border:"1px solid rgba(124,58,237,0.25)" }}>
                      ...
                      <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                        ...
                        {iAmDebtor&&cm.whatsapp&&cm.whatsapp.replace(/\D/g,"").length>=10?(
                          <a href={upiLink(cm,t.amount,trip.name)} target="_blank" rel="noreferrer" style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,background:"linear-gradient(135deg,#7C3AED,#10B981)",border:"none",color:"#fff",borderRadius:10,padding:"0.5rem",textDecoration:"none",fontWeight:800,fontSize:12,fontFamily:"'Outfit',sans-serif",boxShadow:"0 4px 12px rgba(124,58,237,0.4)" }}>💳 Pay Now</a>
                        ):null}
